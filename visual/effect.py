...
from visual.animation import TileAnimation, Animation
...
def create_explosion_effect(game: Game, color: tuple[int, int, int], pos: tuple[int, int]):
...
def create_circle_effect(game: Game, color: tuple[int, int, int], pos: tuple[float, float], size: float, particle_num: int,
						 speed: float):
	animations: list[TileAnimation] = []
	for i in range(particle_num):
...
def play_shout_popup_effect(game: Game, color: tuple[int, int, int], pos: tuple[float, float], size: float, text: str):
	...
	anim = Animation(label.surface, color, (pos[0], pos[1], random.randint(-25, 25), 0.1), (pos[0], pos[1] - 1, 0, size),
